...
(account 0) from a testnet3 wallet listening on `localhost:51472` in several
different languages:
...
func main() {
	...
	conn, err := grpc.Dial("localhost:51472", grpc.WithTransportCredentials(creds))
	if err != nil {
		fmt.Println(err)
		return
	}
...
auto main() -> int {
    ...
    auto channel = grpc::CreateChannel("localhost:51472", creds);
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            var channel = new Channel("localhost:51472", new SslCredentials(cert));
            try
            {
...
var client = new walletrpc.WalletService('localhost:51472', creds);
...
```bash
$ protoc -I /path/to/essentiaone/diviwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    channel = implementations.secure_channel('localhost', 51472, creds)
